...
public class OAuth2AuthorizationRequestBasedOnCookieRepository implements AuthorizationRequestRepository<OAuth2AuthorizationRequest> {
    ...
    public void saveAuthorizationRequest(
        ...
        CookieUtil.addCookie(response, OAUTH2_AUTHORIZATION_REQUEST_COOKIE_NAME, CookieUtil.serialize(authorizationRequest), OAUTH2_AUTHORIZATION_REQUEST_COOKIE_EXPIRE_SECONDS);
    ...
    private void processRedirectUriAfterLogin(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (StringUtils.isNotBlank(redirectUriAfterLogin)) {
            CookieUtil.addCookie(response, REDIRECT_URI_PARAM_COOKIE_NAME, redirectUriAfterLogin, OAUTH2_AUTHORIZATION_REQUEST_COOKIE_EXPIRE_SECONDS);
        }
    }
